...
function fluid_init() {
	...
	// Use window dimensions directly — canvas may report 0 before paint
	const canvas = document.getElementById('fluid-canvas');
	canvas.width  = window.innerWidth;
	canvas.height = window.innerHeight;

	let fluid_bg       = canvas.getAttribute('data-fluid-bg')      || "#02030F",
	    sim_resol      = canvas.getAttribute('data-sim-resol')      || 128,
	    quality        = canvas.getAttribute('data-quality')        || 512,
	    density_disp   = canvas.getAttribute('data-density')        || 0.985,
	    curv_vorticity = canvas.getAttribute('data-vorticity')      || 30,
	    splat_radius   = canvas.getAttribute('data-splat-radius')   || 0.5,
	    backgroundColor = hexToRGB(fluid_bg),
	    transparent    = canvas.getAttribute('data-transparent')
	                       ? JSON.parse(canvas.getAttribute('data-transparent'))
	                       : false;
	...
	let config = {
		...
		SHADING:  false,
		PAUSED:   false,
		BACK_COLOR: { r: backgroundColor.r, g: backgroundColor.g, b: backgroundColor.b },
	...
	// --- Mouse: react to movement anywhere on the page ---
	...
	// --- Click burst: 8 saturated splats around the click point ----
